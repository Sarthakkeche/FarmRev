...
function Addcrop() {
  ...
  const fetchCrops = async () => {
    ...
    const response = await axios.get("https://farmrev-backend.onrender.com/api/crop/mycrops", {
  ...
  const addCrop = async () => {
    const token = localStorage.getItem("token");
    await axios.post(
      "https://farmrev-backend.onrender.com/api/crop/add",
      { name, quantity },
      { headers: { Authorization: token } }
    );
    fetchCrops();
  ...
  const deleteCrop = async (id) => {
    const token = localStorage.getItem("token");
    await axios.delete(`https://farmrev-backend.onrender.com/api/crop/delete/${id}`, {
      headers: { Authorization: token },
    });
    fetchCrops();
  };
